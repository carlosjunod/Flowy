...
/**
 * Wordmark. Display-serif "Tryflowy" + an accent dot. The dot reads as the period in
 * "Tryflowy." and doubles as the brand motif (carried into the login halo and empty
...
export function Brand({ size = 'md', className = '' }: Props) {
  return (
    <span className={`inline-flex items-baseline gap-1.5 font-display tracking-tight ${TEXT_SIZE[size]} ${className}`}>
      <span className="leading-none">Tryflowy</span>
