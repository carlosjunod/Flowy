...
import { verifyAppleIdentityToken, AppleAuthError } from '@/lib/apple-auth';
...
function derivePassword(sub: string): string {
  const secret = process.env.SIWA_PASSWORD_SECRET;
  if (!secret || secret.length < 32) {
    throw new Error('SIWA_PASSWORD_SECRET must be set to a 32+ char value');
  }
...
async function findUserByAppleSub(pb: PocketBase, sub: string): Promise<{ id: string; email: string } | null> {
  try {
    const record = await pb.collection('users').getFirstListItem(`apple_sub = "${sub.replace(/"/g, '')}"`);
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const existing = await findUserByAppleSub(adminPb, identity.sub);
    if (existing) {
      // Rotate password to current HMAC (handles secret rotation transparently).
